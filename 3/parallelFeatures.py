...
# Using the cutoff listed above, split the 50-categories dataset into a 
# training and testing set, with roughly 2/3 of the images in the training set
# and the remaining images in the test set
...
ind = 0
k = 0
# Loop that does the actual partitioning
while k < len(results):
  if k < start + lims[ind]:
    Xtr.append(Xv[k])
    Ytr.append(Yv[k])
...
# test set
numCorrect = 0
for val in range(len(Yte)):
  predVal = intToName[ int( classifier.predict( Xte[val] ) ) ]
  realVal = intToName[ Yte[val] ]
...
unweighted guessing' %(numCorrect, len(Yte), float(numCorrect)/len(Yte), \
(float(numCorrect)/len(Yte)) / (.02) )
